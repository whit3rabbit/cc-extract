...
BUN_NODE_COMPAT_MARKER = "/* ccsilo:bun-node-compat */"

BUN_NODE_COMPAT_PRELUDE = r'''/* ccsilo:bun-node-compat */
;(function(){
  if (typeof globalThis.Bun === "object" && globalThis.Bun && globalThis.Bun.__ccsiloNodeCompat) return;
  ...
  function unsupported(name){return function(){throw new Error("Bun."+name+" is not supported in ccsilo Node runtime");};}
  const semver = {
    order(a,b){
  ...
  const YAML = {
    parse(value){
      ...
      catch { throw new Error("Bun.YAML.parse is not available in ccsilo Node runtime"); }
    },
    stringify(value){
