...
format, registration, and review rules for files in `ccsilo/patches/`.

This is separate from `ccsilo.binary_patcher`, which handles workspace
...
1. Add `ccsilo/patches/<snake_name>.py`.
...
5. If the patch should be user-selectable, add it to
   `CURATED_TWEAK_IDS` in `ccsilo/variants/tweaks.py`.
...
`ccsilo/variants/tweaks.py`. Add the patch ID there when users should be
...
then filters that list against `ccsilo.patches._registry.REGISTRY`, so a
...
from ccsilo.patches.patch_module import PATCH
...
```bash
rtk ruff check ccsilo/patches tests/patches
```

For terminal-visible behavior, add or update gated behavioral tests under
`tests/patches_behavioral/` and verify with `CCSILO_TUI_MCP=1`.
