"""Argparse parser wiring for ``ccsilo``.

Only the parser tree lives here; dispatching is done by ``ccsilo.__main__``
...
def build_parser():
    ...
    paths = subparsers.add_parser("paths", help="Show install and workspace paths")
