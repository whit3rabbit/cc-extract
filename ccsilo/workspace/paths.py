...
def default_workspace_root(
    *,
    ...
    platform_key = platform_key or sys.platform
    home_path = (
        Path(home).expanduser()
        if home is not None
        else Path(env.get("HOME") or "~").expanduser()
    )
    ...
    if platform_key.startswith("win"):
        ...
        if appdata:
            return Path(appdata).expanduser() / APP_DIR_NAME
        return home_path / "AppData" / "Roaming" / APP_DIR_NAME
...
def workspace_root(root: Optional[os.PathLike] = None) -> Path:
    if root is not None:
        return Path(root)
    override = os.environ.get("CCSILO_WORKSPACE")
    if override:
