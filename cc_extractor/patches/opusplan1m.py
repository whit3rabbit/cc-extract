"""Add Opus Plan 1M model alias support."""
...
def _patch_description(js: str) -> str:
    return _replace_first(
        ...
        lambda m: (
            m.group(1)
            + f'if({m.group(2)}==="opusplan[1m]")return"Opus{m.group(3)} in plan mode, else Sonnet{m.group(4)} (1M context)";'
        ),
    )
...
def _patch_label(js: str) -> str:
    return _replace_first(
        ...
        lambda m: m.group(1) + f'if({m.group(2)}==="opusplan[1m]")return"Opus Plan 1M";',
    )
...
def _patch_selector_options(js: str) -> str:
    ...
    new_entry = '{value:"opusplan[1m]",label:"Opus Plan Mode 1M",description:"Use Opus in plan mode, Sonnet (1M context) otherwise"}'
...
def _patch_always_show(js: str) -> str:
    ...
    inject = (
        f'{list_var}.push({{value:"opusplan",label:"Opus Plan Mode",description:"Use Opus in plan mode, Sonnet otherwise"}});'
        f'{list_var}.push({{value:"opusplan[1m]",label:"Opus Plan Mode 1M",description:"Use Opus in plan mode, Sonnet (1M context) otherwise"}});'
    )
...
PATCH = Patch(
    id="opusplan1m",
    name="Opus Plan 1M support",
    group="ui",
    versions_supported=">=2.1.0,<3",
    versions_tested=(">=2.1.0,<2.2",),
    apply=_apply,
    description="Add the opusplan[1m] model alias and keep it visible in the model selector.",
)
