...
def test_try_adhoc_sign_non_darwin_returns_no_codesign(monkeypatch):
    monkeypatch.setattr("ccsilo.binary_patcher.codesign.platform.system", lambda: "Linux")
...
def test_try_adhoc_sign_darwin_handles_missing_codesign(monkeypatch):
    monkeypatch.setattr("ccsilo.binary_patcher.codesign.platform.system", lambda: "Darwin")

    def missing(*args, **kwargs):
        raise FileNotFoundError("codesign")

    monkeypatch.setattr("ccsilo.binary_patcher.codesign.subprocess.run", missing)
...
def test_try_adhoc_sign_darwin_handles_failed_codesign(monkeypatch):
    ...
    monkeypatch.setattr("ccsilo.binary_patcher.codesign.subprocess.run", failed)
...
def test_try_adhoc_sign_terminates_options_before_binary_path(monkeypatch):
    ...
    monkeypatch.setattr("ccsilo.binary_patcher.codesign.subprocess.run", fake_run)
